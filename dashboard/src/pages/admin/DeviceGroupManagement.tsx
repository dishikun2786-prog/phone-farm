...
import { Layers, Plus, Trash2, Edit3, X, GripVertical } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">分组列表 ({groups.length})</h3>
            <button onClick={() => { setEditingGroup({ name: '', deviceIds: [] }); setShowModal(true); }}
              className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">
              <Plus size={14} /> 新建分组
            </button>
          </div>
          {loading ? <p className="text-gray-400 text-sm py-8 text-center">加载中...</p> :
           groups.length === 0 ? <p className="text-gray-400 text-sm py-8 text-center">暂无分组，点击"新建分组"创建</p> :
           <div className="space-y-2">{groups.map(g => (
            <button key={g.id} onClick={() => setSelectedGroup(g)}
              className={`w-full text-left p-3 rounded-lg border transition-all ${selectedGroup?.id === g.id ? 'border-blue-400 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium text-sm">{g.name}</div>
                  <div className="text-xs text-gray-500 mt-0.5">{g.deviceIds.length} 台设备</div>
                </div>
                <button onClick={e => { e.stopPropagation(); handleDelete(g.id); }} className="p-1 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
            </button>
           ))}</div>
          }
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          {!selectedGroup ? (
            <p className="text-gray-400 text-sm py-8 text-center">选择左侧分组查看详情</p>
          ) : (
            <>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-gray-900">{selectedGroup.name}</h3>
                  <p className="text-xs text-gray-500">{selectedGroup.deviceIds.length} 台设备</p>
                </div>
              </div>
              <div className="space-y-2">
                {selectedGroup.deviceIds.length === 0 && <p className="text-gray-400 text-sm py-4 text-center">分组下暂无设备</p>}
                {selectedGroup.deviceIds.map(deviceId => (
                  <div key={deviceId} className="flex items-center justify-between p-2 rounded-lg border border-gray-100">
                    <div>
                      ...
                      <div className="text-xs text-gray-400">{deviceId}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${getDeviceStatus(deviceId) === 'online' ? 'bg-green-500' : 'bg-gray-300'}`} />
                      <button onClick={() => handleRemoveDevice(deviceId)} className="p-1 text-gray-400 hover:text-red-500"><X size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="text-xs text-gray-500 mb-2">可添加的设备:</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div><label className="block text-sm text-gray-600 mb-1">分组名称</label><input value={editingGroup?.name || ''} onChange={e => setEditingGroup(g => ({ ...g, name: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
              <div><label className="block text-sm text-gray-600 mb-1">描述 (可选)</label><input value={(editingGroup as any)?.description || ''} onChange={e => setEditingGroup(g => ({ ...g, description: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            </div>
